...
def page_home():
    ...
    * Соблюдать законы Республики Беларусь в цифровой среде.
    ...
    row3 = st.columns(3)
    _nav_card(row3[0], "🎮", "ИИ против Тебя", "Мини-игра: угадай тип угрозы и сравни свой счёт с тем же ИИ-классификатором.", "ИИ против Тебя", "card_game", badge="НОВОЕ")
    _nav_card(row3[1], "🎯", "Кибер-Полигон", "7-этапный командно-штабной квест — финальная проверка всех знаний.", "Кибер-Полигон", "card_polygon")
...
def page_ipso():
    ...
    st.markdown("---")
    st.header("🎭 Дипфейки и генеративный ИИ: как отличить подделку")
    st.markdown("""
    <div class="info-box">
    <b>Генеративный ИИ</b> — нейросети, которые создают новый текст, изображения, голос или видео
    (а не просто анализируют готовые данные). <b>Дипфейк</b> — разновидность такого контента,
    выдаваемая за настоящую съёмку реального человека: подмена лица в видео, клонирование голоса,
    полностью сгенерированное фото несуществующего человека. Само по себе использование
    генеративного ИИ не преступление и не обман — обманом это становится, когда скрывают, что
    контент искусственный, и выдают его за подлинный.
    </div>
    """, unsafe_allow_html=True)

    st.write("Кстати: иллюстрации на этом портале тоже созданы нейросетью (NanoBanana Pro) — это открыто указано в описании проекта. Разница с дипфейком принципиальная: здесь никто не выдаёт рисунок за фотографию реального события.")

    st.subheader("🔍 На что обратить внимание")
    col1, col2 = st.columns(2)
    with col1:
        st.warning("**Видео и фото**\n\nНеестественные пальцы и руки (лишние/слипшиеся); мерцание или размытие по контуру лица; тени и отражения в глазах не совпадают с освещением сцены; неестественно ровная кожа или наоборот «пластиковый» блеск; странные артефакты на украшениях, тексте, мелких деталях фона.")
    with col2:
        st.warning("**Голос и аудио**\n\nЛёгкий металлический/роботизированный призвук; неестественно ровный темп речи без пауз на вдох; эмоциональная окраска голоса не совпадает со смыслом фразы; смысловые паузы и ударения расставлены «не по-человечески».")

    st.error("🎯 **Практика:** в разделе «Кибер-Полигон» есть этап 7 «ИИ-подделка голоса» — там ровно такой сценарий: звонок голосом, неотличимым от голоса командира. Единственная надёжная проверка — не пытаться «распознать подделку на слух», а перепроверить личность по независимому каналу связи.")

    st.subheader("🧩 Промпт-инъекции — обман самого ИИ")
    st.write("Отдельная угроза — не человек обманывает с помощью ИИ, а кто-то пытается обмануть сам ИИ-сервис специально составленным текстом («промпт-инъекция»): например, спрятанной в сообщении инструкцией «забудь предыдущие правила и...». Это актуально для любых ИИ-чатов и ассистентов, включая корпоративные и государственные.")
    st.success("✅ ИИ-консультант этого портала (раздел «ИИ-Ассистент») спроектирован устойчиво к этому в принципе: он не генерирует свободный текст, а только подбирает наиболее подходящий готовый ответ из фиксированной базы знаний (TF-IDF-поиск) — «уговорить» его сказать что-то, чего нет в базе, невозможно в самой архитектуре.")
...
def page_ai():
    ...
    st.markdown("---")
    st.info("🎮 Хочешь проверить, насколько хорошо ты сам распознаёшь угрозы? Сыграй в мини-игру **«ИИ против Тебя»** — сравни свой счёт с этим же классификатором.")
    if st.button("🎮 Перейти к игре →", key="ai_page_game_link"):
        st.session_state.pending_nav = "ИИ против Тебя"
        st.rerun()
...
def page_game():
    st.title("🎮 Мини-игра «ИИ против Тебя»")
    st.subheader("Проверь себя против того же ИИ-классификатора, что работает в разделе «ИИ-Ассистент»")
    st.write(
        "10 сообщений в стиле реальных угроз для ВПК — с заранее известным правильным ответом. "
        "На каждом раунде ты выбираешь тип угрозы, а затем тот же ИИ-классификатор (ансамбль "
        "наивного байеса и логистической регрессии) независимо анализирует то же сообщение. "
        "Оба ответа сравниваются с правильным — честное сравнение, а не имитация."
    )
    render_ai_game()

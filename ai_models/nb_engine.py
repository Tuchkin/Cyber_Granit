...
# Стандартные русские стоп-слова (предлоги/союзы/местоимения/частицы) — без
# фильтрации короткие вопросы вроде «как дела?» ложно «совпадали» с базой
# знаний просто по общему слову «как». Стоп-слова исключаются ТОЛЬКО из
# TF-IDF-поиска чата, а не из признаков классификаторов (там они не мешают,
# т.к. работают символьные n-граммы, а не отдельные слова).
STOPWORDS = {
    "и", "в", "во", "не", "что", "он", "на", "я", "с", "со", "как", "а", "то", "все", "она",
    "так", "его", "но", "да", "ты", "к", "у", "же", "вы", "за", "бы", "по", "только", "ее",
    "мне", "было", "вот", "от", "меня", "еще", "ещё", "нет", "о", "из", "ему", "теперь",
    ...
    "будет", "ж", "тогда", "кто", "этот", "того", "потому", "этого", "какой", "какая",
    "какие", "каким", "совсем", "ним", "здесь", "этом", "один", "почти", "мой", "тем",
    "чтобы", "нее", "сейчас", "были", "куда", "зачем", "всех", "никогда", "можно", "при",
    "наконец", "два", "об", "другой", "хоть", "после", "над", "больше", "тот", "через",
    "эти", "нас", "про", "всего", "них", "много", "разве", "три", "эту", "моя", "впрочем",
    "хорошо", "свою", "этой", "перед", "иногда", "лучше", "чуть", "том", "нельзя", "такой",
    "им", "более", "всегда", "конечно", "всю", "между", "это", "эта", "тебе", "ах", "дела",
    "делать", "ты", "твой", "твоя", "твои", "умеешь", "можешь", "знаешь",
    "такое", "такая", "такие", "такого", "такому", "такую", "таков", "такова",
}


def _content_tokens(text):
    return [t for t in word_tokens(text) if t not in STOPWORDS and len(t) > 1]


def build_tfidf(kb):
    """kb: список dict с ключами 'q' и 'a'."""
    docs = [_content_tokens(item["q"] + " " + item["q"] + " " + item["a"]) for item in kb]
    df = Counter()
    for tokens in docs:
...
# Светская беседа/офф-топик обрабатываются отдельно, явным распознаванием, а
# не статистическим порогом — надёжнее для самых частых фраз (приветствия,
# благодарность, «как дела», вопросы о самом боте).
SMALLTALK_PATTERNS = [
    "привет", "здравствуй", "здорово", "здарова", "хай ", "добрый день", "добрый вечер",
    "доброе утро", "как дела", "как жизнь", "как поживаешь", "что нового", "как ты",
    "пока", "до свидания", "до встречи", "спасибо", "благодар", "круто", "супер",
    "кто ты", "что ты умеешь", "как тебя зовут", "ты бот", "ты робот", "ты человек",
    "расскажи анекдот", "пошути", "погода", "как погода",
]
SMALLTALK_REPLY = (
    "Я — офлайн ИИ-консультант портала «Кибер-Гранит»: отвечаю на вопросы о кибербезопасности, "
    "законодательстве РБ, ОСИНТ, фишинге и паролях. Спросите, например: «Что делать, если нашёл "
    "флешку?» или «Как включить 2FA?» — или воспользуйтесь кнопками с готовыми вопросами ниже."
)
...
def chat_answer(kb, index, question, threshold=0.2):
    if is_smalltalk(question):
        return {"matched": False, "smalltalk": True, "text": SMALLTALK_REPLY}

    tokens = _content_tokens(question)
    if not tokens:
        return {
            "matched": False, "smalltalk": False,
            "text": "Уточните вопрос — например, про пароли, фишинг, ОСИНТ или законодательство.",
        }
